<x-app-layout class="bg-perhiasan-tua">
                        ...
                        @forelse ($penjualans ?? [] as $item)
                            <tr>
                                ...
                                <td class="p-2 flex flex-row gap-6 justify-center border">
                                    ...
                                    <form method="POST" action="{{ route('perhiasan.tua.penjualan.destroy', $item['id']) }}">
                                        @csrf
                                        @method('DELETE')
    ...
    <div id="editModalPenjualanTua" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-3xl p-6 relative">
            <h3 class="text-lg font-semibold mb-4">Edit Penjualan</h3>
            <form method="POST" id="editFormPenjualanTua" class="space-y-4">
                @csrf
                @method('PUT')
    ...
    <script>
        function openEditModalPenjualan(button) {
            ...
            document.getElementById('editFormPenjualanTua').action = `/perhiasan-tua/penjualan/${penjualan.id}`;
        ...
        function closeEditModalPenjualan() {
            document.getElementById('editModalPenjualanTua').classList.add('hidden');
        }
        setTimeout(() => {
