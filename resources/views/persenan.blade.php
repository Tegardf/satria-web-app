                    ...
                    <table class="min-w-full text-sm text-center border-separate border-spacing-y-2">
                        <thead class="text-gray-600 border-b">
                            <tr>
                                ...
                                <th colspan="2" class="px-4 py-2">Penjualan</th>
                                <th colspan="2" class="px-4 py-2">Pembelian</th>
                            </tr>
                        ...
                        <tbody class="text-gray-700">
                            @forelse($perhiasanTua ?? [] as $row)
                                <tr class="bg-purple-50 rounded-lg">
                                    ...
                                    <td>{{ isset($row['penjualan']['berat']) 
                                            ? number_format($row['penjualan']['berat'], 0, ',', '.') 
                                            : '-' 
                                        }}
                                    </td>
                                    <td>{{ isset($row['penjualan']['harga']) 
                                            ? number_format($row['penjualan']['harga'], 0, ',', '.') 
                                            : '-' 
                                        }}
                                    </td>
                                    <td>{{ isset($row['pembelian']['berat']) 
                                            ? number_format($row['pembelian']['berat'], 0, ',', '.') 
                                            : '-' 
                                        }}
                                    </td>
                                    <td>{{ isset($row['pembelian']['harga']) 
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="py-4 text-center text-gray-400 italic">Data tidak tersedia</td>
                                </tr>
                            @endforelse
                        </tbody>
                        <tfoot class="bg-yellow-100 text-sm font-semibold">
                            <tr>
                                <td class="italic py-2">Total</td>
                                <td>{{ number_format(collect($perhiasanTua)->sum(fn($item) => $item['penjualan']['berat']) ?? 0, 0, ',', '.') }} Gr</td>
                                <td>Rp. {{ number_format(collect($perhiasanTua)->sum('penjualan.harga') ?? 0, 0, ',', '.') }}</td>
                                <td>{{ number_format(collect($perhiasanTua)->sum('pembelian.berat') ?? 0, 0, ',', '.') }} Gr</td>
                                <td>Rp. {{ number_format(collect($perhiasanTua)->sum('pembelian.harga') ?? 0, 0, ',', '.') }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm text-center border-separate border-spacing-y-2">
                        ...
                        <tfoot class="bg-yellow-100 text-sm font-semibold">
                            <tr>
                                <td class="italic py-2">Total</td>
                                <td>{{ number_format(collect($perhiasanMuda)->sum(fn($item) => $item['penjualan']['berat']) ?? 0, 0, ',', '.') }} Gr</td>
                                <td>Rp. {{ number_format(collect($perhiasanMuda)->sum('penjualan.harga') ?? 0, 0, ',', '.') }}</td>
                                <td>{{ number_format(collect($perhiasanMuda)->sum('pembelian.berat') ?? 0, 0, ',', '.') }} Gr</td>
                                <td>Rp. {{ number_format(collect($perhiasanMuda)->sum('pembelian.harga') ?? 0, 0, ',', '.') }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
