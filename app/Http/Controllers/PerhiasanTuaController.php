...
class PerhiasanTuaController extends Controller
{
    ...
    public function pembelianGet() {
        ...
        return view('perhiasan-tua.pembelian',  [
            'items' => $pembeliansRaw, 
    ...
    public function ringkasan() {
        $penjualans = Penjualan::with('stock.produk')
            ->whereHas('stock.perhiasan', fn($q) => $q->where('jenis', 'Perhiasan Tua'))
            ->get();
        ...
        return view('perhiasan-tua.ringkasan', [
            'summary' => $summary,
            'penjualans' => $penjualans,
            'grouped' => $grouped,
            'totalBerat' => $totalBerat,
    ...
    public function gesek() {
        ...
        return view('perhiasan-tua.gesek', compact('perBankData', 'bankSums', 'saldoAwal', 'saldoAkhir'));
    }
    ...
    public function penjualanHarian($id) {
        ...
        // 1. Get the Perhiasan Tua
        $perhiasan = Perhiasan::where('jenis', 'Perhiasan Tua')->first();
        if (!$perhiasan) {
            return abort(404, 'Perhiasan Tua tidak ditemukan.');
        }
        ...
        return view('perhiasan-tua.penjualan_harian', [
            'produks' => $produks,
